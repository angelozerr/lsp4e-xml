...
public class LocalXMLStreamConnectionProvider extends LocalStreamConnectionProvider {
	...
	private URL[] getFreemarkerLanguageServerClassPath() {
		...
		if (bundle == null) {
			throw new RuntimeException("Bundle " + XMLPlugin.PLUGIN_ID + " not found"); //$NON-NLS-1$
		}

		URL languageServerJarURL = bundle.getEntry(LANGUAGE_SERVER_JAR_ENTRY_NAME);
		if (languageServerJarURL == null) {
			throw new RuntimeException(
					"Entity " + LANGUAGE_SERVER_JAR_ENTRY_NAME + " not found in bundle " + XMLPlugin.PLUGIN_ID); //$NON-NLS-1$
		}
